...
//for FIFO circular buffer
#define FIFO_SIZE 11
typedef char DataType;
DataType fifo[FIFO_SIZE];
...
//put data into FIFO, skip if full
//return 1 for success, 0 if FIFO full
...
//return 1 for success, O if the FIFO was empty
...
void ButtonISR(void) {
    ...
    uint32_t gpio_buttons =
            ...
            | (~GPIOPinRead(GPIO_PORTH_BASE, 0xff) & (GPIO_PIN_1)) << 1// Booster S1 button
            | (~GPIOPinRead(GPIO_PORTK_BASE, 0xff) & (GPIO_PIN_6)) >> 3 // Booster S2 button
            | ~GPIOPinRead(GPIO_PORTD_BASE, 0xff) & (GPIO_PIN_4); // Booster joystick select button
...
int getData(int *pt, int *pd)
{
    ...
    else return 0; // no data available
    flag = 0;
}